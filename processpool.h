...
{
private:
    processpool( int listenfd, int process_number = 2 );
public:
...
static int setSockfdattr(int fd, SockfdAttr attr)
{
    ...
    switch (attr)
    {
        case SET_nonblocking:
            ...
            fcntl(fd, F_SETFL, new_option);
            break;
        case SET_closeonexec:
            old_option = fcntl(fd, F_GETFD);
            new_option = old_option | FD_CLOEXEC;
            fcntl(fd, F_SETFD, new_option);
            break;
        default:
            break;
...
static void addfd( int epollfd, int fd )
{
    ...
    //setnonblocking( fd );
    setSockfdattr(fd, SET_nonblocking);
}
...
void processpool< T >::setup_sig_pipe()
{
    ...
    setSockfdattr(sig_pipefd[1], SET_nonblocking);
